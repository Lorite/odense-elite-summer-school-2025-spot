...
def move_hand_to_point_with_gravity_offset(command_client, state_client, xyz_vision,
    ...
    # (Optional) pre-approach 10 cm above the target
            ...
            while True:
                ...
                try:
                    ...
                    xyz_vision, frame, snapshot = pixel_to_xyz_in_frame(
                        image_client, config.image_source, click_xy, out_frame=VISION_FRAME_NAME
                    )
                    print(f"Click {click_xy} → 3D ({frame}): [{xyz_vision[0]:.3f}, {xyz_vision[1]:.3f}, {xyz_vision[2]:.3f}] m")
