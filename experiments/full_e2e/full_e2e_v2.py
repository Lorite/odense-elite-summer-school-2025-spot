...
import experiments.vs_execution.vs as vs
import experiments.vs_execution.grasp as grasp
...
def get_object_profile():
    """Get the object from the environment variable OBJECT_NAME"""
    object_name = os.getenv('OBJECT_NAME', 'cup_upright')
    props = OBJECTS.get(object_name)
...
def arm_object_grasp_with_centering(config):
    ...
    grasp_profile = get_object_profile()
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # 0) Move the body down
                    body_height = -0.30
                    mobility_params = RobotCommandBuilder.mobility_params(body_height=body_height)
                    stand_cmd = RobotCommandBuilder.synchro_stand_command(params=mobility_params)
                    cmd_id = command_client.robot_command(stand_cmd)
                    time.sleep(1.0)
                    ...
                    block_until_arm_arrives(command_client, cmd_id, 30)
                    ...
                    vs.vs(state_client, command_client, 5.0, 0.0, 0.0, grasp_profile.grasp_dist, 0.0, False)
                    grasp.grasp(grasp_profile.dist_to_floor, grasp_profile.gripper_open_fraction, grasp_profile.gripper_max_vel)
                    ...
                    # command_client.robot_command(
                    #     RobotCommandBuilder.claw_gripper_open_fraction_command(
                    #         open_fraction=float(grasp_profile.gripper_open_fraction),
                    #         max_vel=0.6,
                    #         max_torque=float(grasp_profile)
                    #     )
                    # )
                    # time.sleep(3)
